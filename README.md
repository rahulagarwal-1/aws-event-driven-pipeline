...
2. **EventBridge Pipe** reads from SQS and filters out bookings with duration greater than 15 days (filter handled via pipe configuration)
3. **Consumer Lambda** receives filtered records and stores them as CSV files in S3
4. If the Consumer Lambda fails 3 consecutive times, the message is moved to a **Dead Letter Queue**.
